...
/*!
	...
	Midas_39_480x128 (3.9" 480x128 Midas displays)
	ST7282_39_480x128 (3.9" 480x128 displays with ST7282 controller)
...
void RA8875::setFontRotate(boolean rot) {
	rot == true ? _FNCR1_Reg |= (1 << 4) : _FNCR1_Reg &= ~(1 << 4);
	_writeRegister(RA8875_FNCR1,_FNCR1_Reg);
}
...
{
	if (_displaySize == Adafruit_480x272 || _displaySize == Adafruit_800x480 || _displaySize == Midas_39_480x128 || _displaySize == ST7282_39_480x128) GPIOX(on);
	if (on == true){
...
void RA8875::sleep(boolean sleep) 
{
	if (_sleep != sleep){//only if it's needed
		...
		if (_sleep == true){
			...
			if (_displaySize == Adafruit_480x272 || _displaySize == Adafruit_800x480 || _displaySize == Midas_39_480x128 || _displaySize == ST7282_39_480x128/* || _displaySize == Adafruit_640x480*/) GPIOX(false);
			...
			_setSysClock(sysClockPar[_initIndex][0],sysClockPar[_initIndex][1],initStrings[_initIndex][2]);
			//5)turn on backlight
			if (_displaySize == Adafruit_480x272 || _displaySize == Adafruit_800x480 || _displaySize == Midas_39_480x128 || _displaySize == ST7282_39_480x128/* || _displaySize == Adafruit_640x480*/) GPIOX(true);
